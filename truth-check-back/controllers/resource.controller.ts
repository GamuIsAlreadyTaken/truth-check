...
class ResourceController {
  ...
  public static async fetchChain(
    ...
    const resourcesRequested = await this.fetchOne(
      { params, response, state } as RouterContext<string>,
    );
    ...
    for (const foreignKey of foreignKeys) {
      ...
      const referencedResource = await this.fetchOne(
        { params, response, state } as RouterContext<string>,
      );
      ...
      const referencedResourceForeignKeys = findForeignKeys(
        referencedResource,
        foreignKeys,
      );
  ...
  */
  ...
  public static async fetchAll(
    ...
    const visibleResources = resources.filter((r) =>
      r.isPublic || r.authorId == state.id
    );
  ...
  public static async create(
    ...
    if (
      [name, description, imageURI, data, isPublic].some((e) => e === undefined)
    ) {
  ...
  public static async update(
    { request, response, params, state }: RouterContext<string>,
  ) {
    const { id } = params;
  ...
  public static async delete(
    { response, params, state }: RouterContext<string>,
  ) {
    const { id } = params;
    ...
    const deleted = await ResourceService.removeOne(id);
    if ("error" in deleted) return response.status = Status.NotFound;
    return response.status = Status.NoContent;
  }
}
